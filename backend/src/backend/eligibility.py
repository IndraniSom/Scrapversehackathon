...
def evaluate(group: RuleGroup, company: CompanyProfile, as_of: datetime) -> RuleResult:
    ...
    return RuleResult(rule_id=group.id, title="All hard eligibility rules", evaluation=evaluation, explanation=group_explanation(evaluation), company_value=None, requirement_value=None, evidence=[], children=children)

def recommendation_for(result: RuleResult, lifecycle: Literal["OPEN", "CLOSED", "CANCELLED"]) -> Literal["BID", "REVIEW", "NO_BID"]:
...
def _evaluate_node(node: RuleGroup | RuleLeaf | UnsupportedRuleLeaf, company: CompanyProfile, as_of: datetime) -> RuleResult:
...
def _turnover(predicate: TurnoverAveragePredicate, company: CompanyProfile) -> tuple[Evaluation, str | None, str, str]:
    ...
    for year in predicate.required_financial_years:
        ...
        matches = [item for item in rows if item.legal_entity_id == company.bidder_legal_entity_id and item.audited]
        if not matches:
...
def _certification(predicate: CertificationPredicate, company: CompanyProfile) -> tuple[Evaluation, str | None, str | None, str]:
    """Evaluate a named certificate on the predicate's explicit local date."""
    if predicate.valid_at is None:
        return ("UNKNOWN", None, None, "Explicit certification validity anchor is missing.")
    ...
    complete = [(item, item.valid_from, item.valid_until) for item in matches if item.valid_from is not None and item.valid_until is not None]
    valid = [item for item in complete if item[1] <= anchor <= item[2]]
    if valid:
...
def _emd(predicate: EmdPredicate, company: CompanyProfile) -> tuple[Evaluation, str | None, str, str]:
...
def _deadline(predicate: DeadlinePredicate, as_of: datetime) -> tuple[Evaluation, str, str, str]:
...
def _trusted(evidence: list[EvidenceSpan]) -> bool:
    ...
    return all(item.extraction_state == "EVIDENCE_VERIFIED" and item.review_state in {"HUMAN_CONFIRMED", "HUMAN_EDITED"} for item in evidence)

def _leaf_result(node: RuleLeaf | UnsupportedRuleLeaf, evaluation: Evaluation, explanation: str, company_value: str | None = None, requirement_value: str | None = None) -> RuleResult:
...
def leaf_counts(result: RuleResult) -> tuple[int, int, int, int]:
    """Count leaf PASS/FAIL/UNKNOWN/NOT_APPLICABLE for versioned assessment storage."""
    if not result.children:
        return (int(result.evaluation == "PASS"), int(result.evaluation == "FAIL"), int(result.evaluation == "UNKNOWN"), int(result.evaluation == "NOT_APPLICABLE"))
    p = f = u = n = 0
    for child in result.children:
        cp, cf, cu, cn = leaf_counts(child)
        p += cp; f += cf; u += cu; n += cn
    return p, f, u, n
