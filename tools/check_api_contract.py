...
EXAMPLES = {
    "listOpportunities": ("opportunities.manual.json", "OpportunityListEnvelope"),
    "getAssessment": ("assessment.manual.json", "AssessmentViewEnvelope"),
    "getAmendmentImpact": (
        "amendment-impact.manual.json",
        "AmendmentImpactViewEnvelope",
    ),
    "getSourceProof": ("source-proof.manual.json", "SourceProofViewEnvelope"),
}
RESPONSES = (
    ("/health/live", 200, "HealthResponse"),
    ("/health/ready", 200, "HealthResponse"),
    ("/api/v1/opportunities", 200, "OpportunityListEnvelope"),
    ...
    ("/api/v1/source-proof", 200, "SourceProofViewEnvelope"),
    ("/api/v1/opportunities/missing", 404, "ErrorEnvelope"),
)
MODEL_NAMES = {
    "getLiveness": "HealthResponse",
    "getReadiness": "HealthResponse",
    "listOpportunities": "ApiEnvelope[OpportunityList]",
    "getAssessment": "ApiEnvelope[AssessmentView]",
    "getAmendmentImpact": "ApiEnvelope[AmendmentImpactView]",
    "getSourceProof": "ApiEnvelope[VerifiedSourceProof]",
}
...
def _frozen_surface(contract: Mapping[str, object]) -> set[tuple[str, str, str]]:
    ...
    if not isinstance(paths, dict):
        raise ContractCheckError("frozen contract paths are missing")
    surface = set()
    for path, item in paths.items():
        if not isinstance(path, str) or not isinstance(item, dict):
            raise ContractCheckError("frozen contract path is malformed")
        for method, operation in item.items():
            ...
            if not isinstance(operation, dict) or not isinstance(
                ...
                raise ContractCheckError("frozen operation is malformed")
...
def verify_surface(app: FastAPI, contract: Mapping[str, object]) -> None:
    ...
    if actual != expected or generated_surface != expected:
        raise ContractCheckError("concrete route, method, or operation surface drift")
    models = {
        str(route.operation_id): getattr(route.response_model, "__name__", "")
        for route in concrete_routes(app)
    }
    if models != MODEL_NAMES:
        raise ContractCheckError("concrete response-model surface drift")
...
def _schema(contract: Mapping[str, object], name: str) -> Mapping[str, object]:
    ...
    if not isinstance(schema, dict):
        raise ContractCheckError(f"frozen response schema is missing: {name}")
    return schema
...
def _validate_frozen(contract: Mapping[str, object], name: str, instance: object) -> None:
    ...
    if errors:
        raise ContractCheckError(f"frozen response validation failed: {name}")
...
def _verify_examples(app: FastAPI, contract: Mapping[str, object], root: Path) -> None:
    ...
    for operation, (filename, schema_name) in EXAMPLES.items():
        ...
        if operation not in MODEL_NAMES:
            raise ContractCheckError(f"example operation is missing: {operation}")
    ...
    with TestClient(app) as client:
        for path, status, schema_name in RESPONSES:
            ...
            if response.status_code != status:
                raise ContractCheckError(f"unexpected response status: {path}")
...
def main() -> int:
    ...
    print("validated 6 concrete GET operations, response models, 4 examples, and 7 responses")
    return 0
