...
export const generateUploadUrl = mutation({
  args:{},
  handler: async (ctx)=>{ await requireOrganization(ctx); return await ctx.storage.generateUploadUrl(); },
});
...
export const approveDocument = mutation({ args:{documentId:v.id("companyDocuments")}, handler: async (ctx,args)=>{
  const o=await requireOrganization(ctx); const d=await ctx.db.get(args.documentId);
