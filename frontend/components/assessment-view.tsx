...
/** Renders a recommendation summary and its hard-failure or unknown counts. */
function DecisionSummary({ assessment, label }: { assessment: Assessment; label: string }) {
  ...
  return (
    <section className={`decision-summary ${assessment.recommendation === "NO_BID" || uncertain ? "decision-risk" : ""}`} aria-labelledby={headingId}>
      <div><p className="context-label">{label}</p><h2 id={headingId}>{label} recommendation <StatusBadge status={assessment.recommendation} /></h2></div>
...
/** Renders one evaluated rule and every cited source span. */
function RuleResultRow({ result }: { result: RuleResult }) {
  return (
    <li className="rule-result">
      <div className="rule-result-heading"><div><h3>{result.title}</h3><p>{result.explanation}</p></div><StatusBadge status={result.evaluation} /></div>
      ...
      {result.evidence.map((evidence) => <EvidenceDetails key={`${evidence.document_version_id}-${evidence.physical_page_number}-${evidence.normalized_page_text_sha256}`} evidence={evidence} />)}
      {result.children.length > 0 && <ul className="rule-list nested-rule-list">{result.children.map((child) => <RuleResultRow key={child.rule_id} result={child} />)}</ul>}
    </li>
  );
}

/** Renders the company identity and evidence inventory used for evaluation. */
function ProfileSummary({ envelope }: { envelope: AssessmentEnvelope }) {
  ...
  return (
    <section className="profile-summary" aria-labelledby="profile-title">
      <div><p className="context-label">Evaluation profile</p><h2 id="profile-title">{profile.name}</h2></div>
      <dl className="definition-grid">
        <div><dt>Legal entity</dt><dd>{profile.bidder_legal_entity_id ?? "Unverified"}</dd></div>
        <div><dt>Turnover records</dt><dd>{profile.turnover_evidence.length}</dd></div>
        <div><dt>Certifications</dt><dd>{profile.certifications.map((item) => item.name).join(", ") || "None supplied"}</dd></div>
        <div><dt>Completed project records</dt><dd>{profile.projects.filter((item) => item.completion_state === "COMPLETED").length}</dd></div>
      </dl>
    </section>
  );
}

/** Renders every rule outcome for one document version. */
function EvaluationSection({ assessment, label, id }: { assessment: Assessment; label: string; id: string }) {
  return (
    <section aria-labelledby={id}>
      <div className="section-heading-row"><div><h2 id={id}>{label} rule evaluation</h2><p>Evidence-backed outcomes; unknown inputs remain unknown.</p></div></div>
      <ul className="rule-list">{assessment.rule_results.map((result) => <RuleResultRow key={result.rule_id} result={result} />)}</ul>
    </section>
  );
}

/** Renders the complete validated opportunity assessment without scoring it. */
  ...
  return (
    <>
      <header className="page-intro">
        <Link className="text-link back-link" href="/">← Opportunity register</Link>
        <p className="source-line">{opportunity.source} · {opportunity.source_tender_id}</p>
        <h1>{opportunity.title}</h1>
        <p>{opportunity.authority} · <code>{opportunity.data_mode}</code></p>
      </header>
