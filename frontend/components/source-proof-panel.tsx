/**
 * Source proof panel with explicit unavailable state.
 *
 * Uses landmark, badge, and disclosure with keyboard targets.
 */
...
/** Renders proof provenance with honest fallback. */
export function SourceProofPanel({ proof }: { proof: SourceProof }) {
  ...
  return (
    <section className="proof-panel" aria-labelledby="proof-title">
      ...
      <p className="proof-mode">
        <strong>Data mode:</strong> <code>{proof.data_mode}</code>
      </p>
      ...
      <details className="proof-disclosure" aria-label="Source proof details">
        ...
        <dl className="definition-grid">
          {unavailable ? (
            <div>
              <dt>Capture note</dt>
              <dd>Collector identifiers and timestamps are intentionally omitted because they were not captured.</dd>
            </div>
          ) : (
            <>
              <div>
                <dt>Collector</dt>
                <dd>{proof.collector_name}</dd>
              </div>
              <div>
                <dt>Collector configuration</dt>
                <dd>
                  <code>{proof.collector_config_version}</code>
                </dd>
              </div>
              <div>
                <dt>Provider run</dt>
                <dd>
                  <code>{proof.provider_run_id}</code>
                </dd>
              </div>
              <div>
                <dt>Started</dt>
                <dd>{proof.started_at}</dd>
              </div>
              <div>
                <dt>Completed</dt>
                <dd>{proof.completed_at}</dd>
              </div>
              <div>
                <dt>Terminal state</dt>
                <dd>
                  <code>{proof.terminal_state}</code>
                </dd>
              </div>
              <div>
                <dt>Raw snapshot</dt>
                <dd>
                  <code>{proof.raw_snapshot_sha256}</code>
                </dd>
              </div>
              <div>
                <dt>Normalized opportunity</dt>
                <dd>
                  <code>{proof.normalized_record.id}</code>
                </dd>
              </div>
            </>
          )}
        ...
        {!unavailable && (
          <div className="proof-records">
            <p className="proof-linkage">
              Raw snapshot {proof.raw_snapshot_sha256} was normalized as opportunity {proof.normalized_record.id}.
            </p>
