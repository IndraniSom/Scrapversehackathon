/**
 * Amendment impact with landmark and status semantics.
 *
 * Shows transition, authority, documents, and clause comparison.
 */
...
/** Document version with hash and source link. */
function DocumentVersion({ label, id, sha256, sourceUrl }: { label: string; id: string; sha256: string; sourceUrl: string }) {
  return (
    <div className="document-version">
      ...
      <h3>{id}</h3>
      <code className="full-hash">{sha256}</code>
      <a className="text-link" href={sourceUrl} target="_blank" rel="noreferrer">
        Open official document source
      </a>
    </div>
  );
}

/** Clause block with heading and evidence. */
function Clause({ label, evidence }: { label: string; evidence: EvidenceSpan }) {
  return (
    <section className="clause-block" aria-labelledby={`${label}-clause`}>
      <p className="context-label">{label}</p>
      <h3 id={`${label}-clause`}>{evidence.section_heading}</h3>
  ...
  return (
    <>
      <header className="page-intro">
        <Link className="text-link back-link" href={`/opportunities/${encodeURIComponent(impact.opportunity_id)}`}>
          ← Opportunity assessment
        </Link>
        <p className="source-line">
          Amendment evidence · <code>{impact.data_mode}</code>
        </p>
        <h1>Amendment impact</h1>
        <p>
          Rule <code>{impact.changed_rule_id}</code> is traced to an identified authority statement.
        </p>
      </header>
      <section className={`decision-summary ${impact.authority_change_applied ? "decision-change" : ""}`} aria-label="Recommendation transition">
        <div>
          <p className="context-label">Base recommendation</p>
          <StatusBadge status={impact.base_recommendation} />
        </div>
        <span className="transition-arrow" aria-hidden="true">
          →
        </span>
        <div>
          <p className="context-label">Amended recommendation</p>
          <StatusBadge status={impact.amended_recommendation} />
        </div>
      ...
      <section className="authority-block" aria-labelledby="authority-title">
        <div className="section-heading-row">
          <div>
            <h2 id="authority-title">Authority statement</h2>
            <p>Actor and disposition determine whether the change can affect the recommendation.</p>
          </div>
        </div>
        <dl className="definition-grid">
          <div>
            ...
            <dd>
              <StatusBadge
                status={statement.actor === "AUTHORITY" ? "ACCEPTED" : statement.disposition === "REJECTED" ? "REJECTED" : "UNCHANGED"}
          ...
          <div>
            <dt>Disposition</dt>
            <dd>{statement.disposition}</dd>
          </div>
          <div>
            <dt>Replaces</dt>
            <dd>{statement.replaces_document_id ?? "No document replacement"}</dd>
          </div>
          <div>
            <dt>Effect</dt>
      ...
      <section aria-labelledby="documents-title">
        <div className="section-heading-row">
          <div>
            <h2 id="documents-title">Document lineage</h2>
            <p>Hashes identify the exact base and amendment content assessed.</p>
          </div>
        </div>
      ...
      <section aria-labelledby="clause-title">
        <div className="section-heading-row">
          <div>
            <h2 id="clause-title">Clause comparison</h2>
            <p>Quoted source text remains attached to extraction and review state.</p>
          </div>
        </div>
        <div className="clause-comparison">
          <Clause label="Old" evidence={impact.old_clause} />
          <Clause label="New" evidence={impact.new_clause} />
        </div>
      </section>
    </>
  );
